...
// because gojs has `window` and window is not defined when gatsby builds
let initDiagram
let myDiagram
try {
  const go = require('gojs')

  initDiagram = function(data, selectedItem) {
    const $ = go.GraphObject.make;
    myDiagram =
      $(go.Diagram, "go-js-diagram",
        {
    ...
    myDiagram.nodeTemplate =
      $(go.Node, "Vertical",
        ...
        $(go.TextBlock,
          { font: "bold 12px sans-serif", spacingBelow: 3, textAlign: "center", maxSize: new go.Size(100, NaN) },
  ...
  function onNodeClick(_, obj) {
    var clicked = obj.part
    clicked.Yd.onNodeClick()
  }
    ...
    for (var i = 0; i < nodeDataArray.length; i++) {
      ...
      if (mother !== undefined && father !== undefined) {
        ...
        if (link === null) {
          ...
          if (typeof window !== `undefined` && window.console) window.console.log("unknown marriage: " + mother + " & " + father);
          continue;
        }
        var mdata = link.data;
